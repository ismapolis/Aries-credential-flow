...
export async function checkPreviusCredential(subject, type, context) {
    ...
    try {
        const credencials = await context.agent.dataStoreORMGetVerifiableCredentials({
            where: [
                { column: "subject", value: [subject] },
                { column: "type", value: [type] },
            ],
        });
        console.log("Found: " + credencials.length);
        if (credencials.length > 0) {
            const id = credencials[-1].verifiableCredential.id;
...
export async function createOfferCredential(preview, attach, context) {
    ...
    console.log("Attributes from credential~preview: " + previewAttributes.toString());
